...
def scikit_fbp(sinogram, theta):
	...
	t = time.time()
	rebuild_image = ski.iradon(sinogram, theta, None, "ramp", "linear", True)
	t = time.time() - t
	
	return rebuild_image, t

def scikit_sart(sinogram, theta):
	#~ start_angle = param["parallel_geometry"]["start_angle"]
	#~ end_angle = param["parallel_geometry"]["end_angle"]
	#~ nb_angles = param["parallel_geometry"]["nb_angles"]
	...
	t = time.time()
	rebuild_image = ski.iradon_sart(sinogram, theta, relaxation=0.15)
	t = time.time() - t
